...
/**
 * Created by vfatu on 31.01.2017.
 */
...
public class MraidJavascript {
    ...
    public static final String JAVASCRIPT_SOURCE =
            "(function() {\n" +
                    ...
                    "      //bridge.executeNativeCall(CalendarEventParser.arguments); //TODO bchitu: remove arg name!! Calendar not suppoeted yet. moving on for now\n" +
                    "    } else {\n" +
                    ...
                    "    if((supportProperties['storePicture'])=='false'){"+
                                "return;" +
                         "}" +
                    "    if (!mraid.isViewable()) {\n" +
    ...
    public static final String FILTERED_JAVASCRIPT_SOURCE = JAVASCRIPT_SOURCE
            .replaceAll("(?m)^\\s+", "")
            .replaceAll("(?m)^//.*(?=\\n)", "");
}
